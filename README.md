...
```bash
podman build -t ehdlaos/test:v1 .
```
...
```bash
# 기본 실행 (포트 8080으로 접속)
podman run -d -p 8080:80 --name mycontainer ehdlaos/test:v1
...
# 이미지 삭제
podman rmi ehdlaos/test:v1

# 태그 변경 후 Push
podman tag ehdlaos/test:v1 ehdlaos/test:latest
